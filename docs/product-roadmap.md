...
3. Mission grammar and architecture hardening
   ...
   - Harden the F5 map editor/tuner only where it helps internal route and balance proof; keep save writes guarded by diff preview and keep modder packaging as later scope.
...
- harden the in-game F5 map editor/tuner as the internal terrain and marker authoring aid, including region/marker/object inspection, visible creation tools, resize handles, legend clarity, optional pathing-blocked sampling, clipboard snippets, guarded diff-preview source saves, validation, undo/redo, mission switching, and in-memory mission re-init
